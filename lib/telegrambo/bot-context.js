...
export default function(request) {
  ...
  self.on = (eventName, eventHandler) => {
    ...
    if (EVENTS.has(eventName))
      ...
      EVENTS.get(eventName) = [eventHandler];
  };
  ...
  function runEventHandlers(trigger, eventName, eventPayload) {
    console.log('//run event handler: ', eventName);
    console.log('Event has handlers: ', EVENTS.has(trigger));
    const result = [];
    if (EVENTS.has(trigger)) {
      const context = new EventContext(eventName, eventPayload);
      for (let handler of EVENTS.get(trigger)) 
        result.concat(handler(context, eventName));
    }
